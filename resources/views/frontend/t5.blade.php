...
<body>
<div class="markdown"><div class="lesson-num p">Lesson5</div><h1 id="bootstrap">Bootstrap
</h1>
...
<p>スマートフォンやタブレットの普及によって、これまでパソコンを使用する人しか馴染みのなかったインターネットを誰もが使うようになり、それに伴い様々なWebサービスをたくさんの人へ提供できる環境になりました。その事から、「いかに効率良く開発・制作」を行い、「アイデアをすぐ形にして世の中にリリースする」ことの重要性が高まってきているのではないでしょうか。</p>
...
</h2><div class="subsection"><p><a href="http://getbootstrap.com/" target="_blank">Bootstrap</a>はHTML、CSS、JavaScriptからなる巨大なライブラリです。元々は米Twitter社で開発された経緯から、Twitterのような見た目のサイトを作るための部品が数多く含まれています。Bootstrapをページに組み込むと、ページの見栄えを一気に底上げでき、美しく整ったページを簡単に作れます。また、後ほど解説するレスポンシブデザインにも対応しており、デスクトップからモバイルまで幅広い画面サイズに対応したサイトを楽に作ることができます。</p>
...
<ul>
  <li><a href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.css" target="_blank">BootstrapのCSS: https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.css</a></li>
  <li><a href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.js" target="_blank">BootstrapのJavaScript: https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.js</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/getting-started/browsers-devices/#supported-browsers" target="_blank">Bootstrap対応ブラウザ一覧</a></li>
...
<p>Bootstrapは公式サイト自体がサンプルコード集となっており、最も充実したドキュメントサイトです。</p>
...
<p>公式は英語なので、英語が苦手だと辛い方もいらっしゃるでしょう。更に解説もちゃんと読みたいとなると英語を読むのは苦痛でしょう。そこで非公式の日本語訳ドキュメントも存在します。どうしても英語が厳しい方は、こちらを参照してみてください。</p>
...
</h3><p>CDN（Content Delivery Network）とは、Bootstrapのような公開ライブラリを誰でも自由に使えるよう、Web上に設置（ホスティング）してくれているサイトです。以下のようにHTMLを追加するだけですぐ使えます。CSSとJavaScriptを両方とも入れます。</p>

<p><a href="https://getbootstrap.com/docs/4.3/getting-started/introduction/" target="_blank">こちら</a>の「Quick start」ならびに「Starter template」のサンプルコードをコピーして使うことができます。ただし <code>lang="en"</code> となっている部分は <code>lang="ja"</code> としてください。</p>
...
<p>Bootstrapを使うときは上記のテンプレートを使うようにしましょう。本レッスンでもこれから使用していきます（補足：「Starter template」のコードに記載されている <code>integrity</code> と <code>crossorigin</code> の属性情報は、削除しても動作します）。</p>
...
<p>レスポンシブデザインの基本はメディアクエリです。Bootstrapの内部ではメディアクエリが適切に設定されており、グリッドなどBootstrapの仕組みの中でサイトを作るだけで自動的にレスポンシブデザインのページができあがります。ウインドウ幅を変えたとき、不自然な表示になっていないか逐一確認しながら制作していきましょう。</p>

<p>レスポンシブデザインもBootstrapが用意してくれているので、私たちはそれを利用させてもらえれば良いのです。そのために理解する必要のある技術がグリッドシステムでした。</p>
...
<p>なお、<code>min-width</code> の他に「○○○px以下の場合」を指定できる <code>max-width</code> もあります。</p>
...
<p>実際に試してみましょう。先ほどのBootstrap Templateから、以下の内容で <em>bootstrap_lesson.html</em> をCloud9上に作成してください。これはテスト用のファイルなのでどこに作成してもらっても構いません。</p>
...
<p>上記のHTMLを開くと、ウインドウ幅が768px以上のときはdivが左右に並びます。</p>
  ...
  <div class="code"><pre>    <span class="tag">&lt;body&gt;</span>
        <span class="tag">&lt;div</span> <span class="attribute-name">class</span>=<span class="string"><span class="delimiter">"</span><span class="content">container</span><span class="delimiter">"</span></span><span class="tag">&gt;</span>
            <span class="tag">&lt;div</span> <span class="attribute-name">class</span>=<span class="string"><span class="delimiter">"</span><span class="content">row</span><span class="delimiter">"</span></span><span class="tag">&gt;</span>
                ...
                <span class="tag">&lt;div</span> <span class="attribute-name">class</span>=<span class="string"><span class="delimiter">"</span><span class="content">offset-md-2 col-md-5</span><span class="delimiter">"</span></span><span class="tag">&gt;</span>md-offset-2 col-md-5<span class="tag">&lt;/div&gt;</span>
            <span class="tag">&lt;/div&gt;</span>
        <span class="tag">&lt;/div&gt;</span>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/layout/grid/" target="_blank">参考: グリッドシステム</a></li>
</ul>
</div></section><section id="chapter-6"><h2 class="index">6. Bootstrapのパーツ
</h2><div class="subsection"><p>ここでは、 Bootstrapのパーツのいくつか重要なものを紹介します。他にもパーツはあるので、公式サイトを眺めるだけでもしておくと良いでしょう。</p>

<p>HTMLの見た目の部分をBootstrapのようにキレイにするには、基本的に <code>class</code> を指定してあげればOKです。</p>

<p>ここからはBootstrapを利用したサンプルコードをいくつか見ていきます。先ほどのBootstrap Templateのbody内部にサンプルコードをコピペしてPreviewして試してください。また、 bootstrapの <code>class="table"</code> を逆に削除してみたらどういう表示になるかなども確認しておくと一層理解が深まるでしょう。Cloud9でやるのが面倒な方は、 JS Binもいくつか用意してあるので、それを編集して遊んでみてもOKです。</p>
...
<p>また、テーブルの見た目の種類として、以下のクラスを追加できます。</p>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/content/tables/" target="_blank">参考: テーブル</a></li>
</ul>
...
<p>指定の要素の前後にアクセサリをつけることができます。まず、アクセサリをつけたい部品の親要素に <code>input-group</code> クラスをつけてください。次に、アクセサリをつけたい要素自身に <code>input-group-prepend</code> クラスをつけることで左端（要素の前）に、<code>input-group-append</code> クラスをつけることで右端（要素の後ろ）に、アクセサリを追加できます。アクセサリの中に表示したい文字列は <code>&lt;span&gt;</code> 要素で囲い、 <code>input-group-text</code> クラスを追加してください。</p>

<p>なお、前後で同時にアクセサリをつけることはできますが、片側に複数のアクセサリを連続してつけることはできません。</p>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/forms/" target="_blank">参考: フォーム</a></li>
  <li><a href="https://getbootstrap.com/docs/4.3/components/input-group/" target="_blank">参考: インプットグループ</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/buttons/" target="_blank">参考: ボタン</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/dropdowns/" target="_blank">参考: ドロップダウン</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/navs/#tabs" target="_blank">参考: タブ</a></li>
</ul>
...
<ul>
  <li>リスト( <code>&lt;ul&gt;</code> 要素)を追加し、各メニュー項目はリストアイテム( <code>&lt;li&gt;</code> 要素)にします。また、各メニューはリンクとして設定します。さらに、この <code>&lt;ul&gt;</code> を囲う親要素の <code>&lt;div&gt;</code> を記述します。</li>
...
<ul>
  ...
  <li>これだけだと、会社情報のドロップダウンをクリックしても何も表示されません。そこで、ドロップダウンにした <code>&lt;a&gt;</code> 要素へ <code>data-toggle="dropdown"</code> という属性を追加してください。</li>
</ul>
...
<ul>
  ...
  <li>3本線のボタンを用意するには、以下のような <code>&lt;button&gt;</code> 要素を追加してください。この <code>&lt;button&gt;</code> をサイト名のリンク( <code>&lt;a href="#" class="navbar-brand"&gt;</code> ) の直前に書くとボタンは左側、リンクの直後に書くとボタンは右側に表示されます。今回の例は右側へ表示するようにしています。</li>
</ul>
...
<ul>
  <li><code>&lt;ul class="navbar-nav"&gt;</code> の要素を囲う親要素の <code>&lt;div&gt;</code> を記述してください。その <code>&lt;div&gt;</code> に <code>collapse</code> と <code>navbar-collapse</code> クラスを追加してください。さらに <code>id="nav-bar"</code> というid属性を追加しましょう。ここでは <code>nav-bar</code> というIDにしましたが、これは3本線の <code>&lt;button&gt;</code> で <code>data-target="#nav-bar"</code> と設定したことによります。<code>data-target</code> の設定内容を変更している場合は、その内容と <code>&lt;div&gt;</code> のID属性が同じ名前となるようにしてください。（<code>data-target</code> の方は、先頭にある <code>#</code> が必須です。）</li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/navbar/" target="_blank">参考: ナビゲーションバー</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/badge/" target="_blank">参考: バッジ</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/badge/#pill-badges" target="_blank">参考: ピルバッジ</a></li>
</ul>
...
<p>例えばユーザ登録のフォームを送信した時、登録に成功したか、失敗したかを表示するのに使います。主に使うのは、青( <code>primary</code> )、緑( <code>success</code> )、黄色 ( <code>warning</code> )、赤 ( <code>danger</code> ) ですが、ボタンやバッジで使う色( <code>secondary</code>, <code>info</code>, <code>light</code>, <code>dark</code> )はひととおり揃っています。</p>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/alerts/" target="_blank">参考: 警告</a></li>
</ul>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/components/card/" target="_blank">参考: カード</a></li>
</ul>
...
<p><a href="https://getbootstrap.com/docs/4.3/utilities/spacing/" target="_blank">参考: 余白</a></p>
...
<p><a href="https://getbootstrap.com/docs/4.3/utilities/borders/" target="_blank">参考: 罫線</a></p>
...
<p><a href="https://getbootstrap.com/docs/4.3/utilities/text/#text-alignment" target="_blank">参考: 文字の配置</a></p>
...
<p><a href="https://getbootstrap.com/docs/4.3/utilities/flex/" target="_blank">参考: Flexboxモデル</a></p>
...
<p>Font Awesomeを利用する際は、BootstrapのHTMLテンプレートの <code>&lt;body&gt;</code> の下方、<code>&lt;script&gt;</code> の要素が多く記述されている場所に以下の1行を追加してください（今までのサンプルコードでも、アイコン自体は使っていませんが、以下の1行は入れています）。</p>
...
<p>Font Awesomeで用意されているアイコンの全ては <a href="https://fontawesome.com/icons?d=gallery&amp;m=free" target="_blank">コチラのページで確認できます</a> 。実に多くのアイコンが用意されていて、先ほどのページでは全てを一覧表示しきれないので「↓ Load more icons…」ボタンクリックして次の一覧を表示したり、画面上側の検索機能や左側の絞り込み機能を利用したりしてアイコンを確認してください。一覧の中で使いたいアイコンが見つかったら、そのアイコンをクリックしてください。以下の例ではdownloadのアイコンを使ってみます。</p>

<p><img src="https://techacademy.s3.amazonaws.com/bootcamp/frontend/bootstrap/5-faicon4_01.png" alt="使いたいアイコンをクリックする"></p>

<p>downloadアイコンの詳細ページへ遷移します。画面上部に、downloadアイコンを利用するための記述方法(iタグ)が表示されていますので、タグの文字列にカーソルを合わせます。「Click to Copy HTML」と表示されたらクリックして、該当の記述をクリップボードにコピーしてください。</p>

<p><img src="https://techacademy.s3.amazonaws.com/bootcamp/frontend/bootstrap/5-faicon4_02.png" alt="アイコンの記述をコピーする"></p>

<p>コピーした記述を、使いたい場所にペーストしてください。以下はBootstrapで作ったボタンの中でdownloadアイコンを表示させています。</p>
...
<ul>
  <li><a href="https://getbootstrap.com/docs/4.3/extend/icons/" target="_blank">参考: Icons</a></li>
</ul>
